...
def ShowSensor1(ID):
    with sqlite3.connect("TempAndHumid.db") as con:
        ...
        if(data[4] == "ON"):
            ...
            StatusAuto = False
        send_data = {'Id':data[0],'Temp':data[1],'Humid':data[2],'StatusOnOff':StatusOnOff,'StatusAuto':StatusAuto}

    return jsonify(send_data)


@app.route("/IoT/Sensor1/<temp>/<humid>")
def CreateSensor(temp,humid):
    with sqlite3.connect("TempAndHumid.db") as con:
        curr = con.cursor()
        sql_cmd ="""
        INSERT INTO SENSOR(Temp,Humid) VALUES(?,?);
        """
        curr.execute(sql_cmd,(int(temp),int(humid)))
        con.commit()
    return jsonify({"message":"success"})
...
def ShowListView(ID):
    with sqlite3.connect("TempAndHumid.db") as con:
        ...
        """
        sensor_pack = [i for i in curr.execute(sql_cmd,(ID,))][0][0]
        sensor_list = sensor_pack.split(" ")
        print(sensor_pack)
        print(sensor_list)
        # args=[1,2]
        sql="SELECT * FROM sensor WHERE id IN ({seq})".format(seq=','.join(['?']*len(sensor_list)))
